<?php
	...
	if ($validacion) {
		...
		$sqlNotificacion =  sprintf("SELECT * FROM tbl_empleados_notificaciones 
										...
										INNER JOIN tbl_prestamos ON en_emp_id=pr_emp_id AND en_usu_id=pr_usu_id 
		...
		while( $row_sqlNotificacion = mysqli_fetch_assoc($rs_sqlNotificacion) ){
			...
			$result["monto"] = $row_sqlNotificacion["pr_monto"];
			$result["cuotas"] = $row_sqlNotificacion["pr_cuotas"];
		}
